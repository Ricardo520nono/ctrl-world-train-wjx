...
if [[ "${DRY_RUN}" == "1" ]]; then
  ...
  echo "[DRY_RUN] gpu=${GPU} port=${POLICY_PORT}"
...
echo "[INFO] Running open-loop first on GPU=${GPU}"
CUDA_VISIBLE_DEVICES="${GPU}" "${CTRLWORLD_PYTHON}" scripts/replay_clean_expert_autoreg.py \
  --ckpt "${WM_CKPT}" \
  --latent_root "${LATENT_ROOT}" \
...
echo "[INFO] Starting QwenOFT policy server task=${TASK} gpu=${GPU} port=${POLICY_PORT}"
(
  cd "${STARVLA_ROOT}"
  export REPO_ROOT="${STARVLA_ROOT}"
  export STARVLA_PYTHON
  export ROBOTWIN_USE_BF16="${ROBOTWIN_USE_BF16:-1}"
  bash examples/Robotwin/eval_files/run_policy_server.sh "${POLICY_CKPT}" "${GPU}" "${POLICY_PORT}"
) > "${SERVER_LOG}" 2>&1 &
SERVER_PID=$!
...
echo "[INFO] Running policy closed-loop on GPU=${GPU}"
CUDA_VISIBLE_DEVICES="${GPU}" "${CTRLWORLD_PYTHON}" scripts/replay_policy_server_autoreg.py \
  --ckpt "${WM_CKPT}" \
  --latent_root "${LATENT_ROOT}" \
  --manifest "${MANIFEST_PATH}" \
  --stat "${STAT_PATH}" \

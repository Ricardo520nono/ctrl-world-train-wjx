...
export LATENT_ROOT="${LATENT_ROOT:-${PUBLIC_ROOT}/latents/action_following_cur1f32_mix4_${TASK}_rot6d20_cfstatemajor_incremental_20260718}"
export META_ROOT="${META_ROOT:-${PUBLIC_ROOT}/dataset_meta_info/action_following_cur1f32_mix4_${TASK}_rot6d20_cfstatemajor_incremental_20260718}"
...
if [[ "${DRY_RUN:-0}" == "1" ]]; then
  ...
  echo "[DRY_RUN] num_shards=${NUM_SHARDS} local_gpu_count=${LOCAL_GPU_COUNT}"
...
if [[ -f "${LATENT_ROOT}/.incremental_complete" ]]; then
  echo "[ERROR] incremental cache is already complete; refusing to overwrite: ${LATENT_ROOT}" >&2
  exit 2
fi
...
"${PYTHON_BIN}" "${CODE_ROOT}/scripts/prepare_action_following_incremental_cache.py" plan \
  ...
  --out_root "${LATENT_ROOT}" | tee "${LOG_ROOT}/incremental_plan.log"

rm -f \
  "${MANIFEST_ROOT}"/enhanced_train_delta_shard*.jsonl \
  "${MANIFEST_ROOT}"/test_quick_delta_shard*.jsonl \
  "${LATENT_ROOT}"/precompute_summary_delta_shard*.json \
  "${LATENT_ROOT}/.incremental_manifests_ready"
...
echo "[INFO] incremental cache complete: ${LATENT_ROOT}"

...
export RUN_NAME="${RUN_NAME:-ACWM_ctrlworld_mix4_dump_bin_bigbin_c32_cur1f32_rot6d20_cfstatemajor_8gpu_20260718}"
export TASKS="dump_bin_bigbin"
export PUBLIC_ROOT="${PUBLIC_ROOT:-/mnt/dataset/public_data/cscsx_projects/AF3/ctrl-world}"
export LATENT_ROOT="${LATENT_ROOT:-${PUBLIC_ROOT}/latents/action_following_cur1f32_mix4_dump_bin_bigbin_rot6d20_cfstatemajor_incremental_20260718}"
export META_ROOT="${META_ROOT:-${PUBLIC_ROOT}/dataset_meta_info/action_following_cur1f32_mix4_dump_bin_bigbin_rot6d20_cfstatemajor_incremental_20260718}"
...
for required in \
  ...
  if [[ ! -f "${required}" ]]; then
    echo "[ERROR] prerequisite missing; run the dump_bin_bigbin incremental precompute first: ${required}" >&2
    exit 2
  fi
done

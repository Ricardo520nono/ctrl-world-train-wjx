...
# Ctrl-World ActionFollowingData clean training after clean LeRobot frame-index fix.
...
# 50-task training uses the task set encoded in the precomputed manifest.
export RUN_NAME="${RUN_NAME:-ACWM_ctrlworld_action_following_clean_50task_chunk32_8gpu_current1_future32_fulldesc_rot6d20_frameidxfix_20260711}"
...
export LATENT_ROOT="${LATENT_ROOT:-/mnt/dataset/public_data/cscsx_projects/ctrl_world_train/latents/action_following_current1_future32_clean_enhanced_explore_50task_fulldesc_rot6d20_frameidxfix_20260711}"
export META_ROOT="${META_ROOT:-/mnt/dataset/public_data/cscsx_projects/ctrl_world_train/dataset_meta_info/action_following_current1_future32_clean_only_50task_fulldesc_rot6d20_frameidxfix_20260711}"
...
export MASTER_PORT="${MASTER_PORT:-29732}"
...
if [[ ! -f "${LATENT_ROOT}/manifests/clean_train.jsonl" ]]; then
  echo "[ERROR] required manifest missing: ${LATENT_ROOT}/manifests/clean_train.jsonl" >&2
  exit 2
fi

...
export RUN_NAME="${RUN_NAME:-ACWM_ctrlworld_mix4_place_dump_c32_cur1f32_rot6d20_cfstatemajor_rgbfix1_8gpu_80k_20260719_debug}"
...
export META_ROOT="${META_ROOT:-${PUBLIC_ROOT}/dataset_meta_info/action_following_cur1f32_mix4_place_dump_rot6d20_cfstatemajor_rgbfix1_20260719_debug}"
...
if [[ "${DRY_RUN:-0}" == "1" ]]; then
  ...
  echo "[DRY_RUN] sampler=task_balanced_mix4 task=50/50 family_per_task=50/12.5/12.5/12.5/12.5"
...
for required in \
  ...
  if [[ ! -f "${required}" ]]; then
    echo "[ERROR] merged RGB-fix prerequisite missing: ${required}" >&2
    exit 2
  fi
done
"${PYTHON_BIN}" "${CODE_ROOT}/scripts/merge_action_following_task_caches.py" \
  ...
  --audit_out "${LATENT_ROOT}/merged_cache_audit.json"

...
class Dataset_mix(Dataset):
    ...
    def __getitem__(self, index):
        ...
        use_abs_joint = getattr(self.args, 'use_abs_joint_action', False)
        dataset_name_for_sample = sample.get('dataset_name', '') if isinstance(sample, dict) else ''
        dataset_name_for_sample = str(dataset_name_for_sample).lower()
        if ('robotwin' in dataset_name_for_sample) or ('aloha' in dataset_name_for_sample) or use_abs_joint:
            ...
            # clean50 abs-joint is 6D; trim 7D sources (e.g., droid subset) to keep shape aligned.
            if action.shape[-1] > 6:
                action = action[..., :6]
        else:
            cartesian_pose = np.array(label['observation.state.cartesian_position'])[state_id]
        ...
        target_action_dim = int(getattr(self.args, 'action_dim', action.shape[-1]))
        if action.shape[-1] > target_action_dim:
            action = action[..., :target_action_dim]
        elif action.shape[-1] < target_action_dim:
            pad_width = target_action_dim - action.shape[-1]
            action = np.pad(action, ((0, 0), (0, pad_width)), mode='constant')

        state_p01_use = state_p01[..., :target_action_dim]
        state_p99_use = state_p99[..., :target_action_dim]

        action = self.normalize_bound(action, state_p01_use, state_p99_use)
